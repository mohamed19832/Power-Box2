...
export default function OfferPricing() {
  const [offerData, setOfferData] = useState<OfferPricingData>(defaultOfferPricingData);
  ...
  const updateFeature = (index: number, value: string) => {
    setOfferData(prev => ({
      ...
      offer_card: {
        ...prev.offer_card,
        features: prev.offer_card.features.map((feature, i) => i === index ? value : feature)
      }
    }));
  };
  ...
  const updateGuarantee = (index: number, value: string) => {
    setOfferData(prev => ({
      ...
      offer_card: {
        ...prev.offer_card,
        guarantees: prev.offer_card.guarantees.map((guarantee, i) => i === index ? value : guarantee)
      }
    }));
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-900">Offer / Pricing Section</h2>
        <Button 
          onClick={handleSave} 
          disabled={isSaving}
      ...
      <Card className="border-2 border-orange-100">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleOfferCardChange('product_name', e.target.value)}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleOfferCardChange('old_price', e.target.value)}
                placeholder="$42.99"
                className="mt-1"
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleOfferCardChange('new_price', e.target.value)}
                placeholder="$31.95"
                className="mt-1"
              />
            </div>
          ...
          <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
            <Label className="text-sm font-medium text-blue-800 mb-3 block">Call-to-Action Button</Label>
        ...
        <CardContent>
          <div className="p-6 bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 rounded-lg">
            <div className="text-center mb-6">
              <Badge variant="secondary" className="mb-4">{offerData.promo_text}</Badge>
              <h3 className="text-2xl font-bold mb-2">{offerData.title}</h3>
              <p className="text-gray-600">{offerData.subtitle}</p>
            </div>
            
            <div className="bg-white rounded-lg p-6 shadow-lg max-w-md mx-auto">
              <h4 className="font-semibold mb-3">{offerData.offer_card.product_name}</h4>
              <div className="flex items-center gap-2 mb-4">
                <span className="text-gray-500 line-through">{offerData.offer_card.old_price}</span>
                <span className="text-green-600 font-bold text-xl">{offerData.offer_card.new_price}</span>
              </div>
